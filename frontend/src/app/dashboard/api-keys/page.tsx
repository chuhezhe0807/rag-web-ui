...
export default function APIKeysPage() {
  ...
  const fetchAPIKeys = async () => {
    try {
      const data = await api.get("/api/ai/api-keys");
      setApiKeys(data);
    } catch (error) {
      toast({
        title: "Error",
  ...
  const createAPIKey = async () => {
    ...
    setIsCreating(true);
    try {
      const data = await api.post("/api/ai/api-keys", {
        name: newKeyName,
        is_active: true,
      });
  ...
  const deleteAPIKey = async (id: number) => {
    try {
      const response = await api.delete(`/api/ai/api-keys/${id}`);
  ...
  const toggleAPIKeyStatus = async (id: number, currentStatus: boolean) => {
    try {
      const response = await api.put(`/api/ai/api-keys/${id}`, {
